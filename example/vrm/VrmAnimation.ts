...
const requiredHumanBoneNames = new Set<VRMHumanBoneName>(
  Object.values(VRMRequiredHumanBoneName)
);
...
export function retargetHumanoidAnimationClips(
  ...
  return clips
    ...
    .map((clip) => retargetHumanoidAnimationClip(clip, context));
}
...
function retargetQuaternionTracks(
  ...
  for (let frame = 0; frame < firstTrack.times.length; frame += 1) {
    ...
    for (const sourceBone of context.sourceBones) {
      const restWorld = context.sourceRestWorld.get(sourceBone.sourceName)!;
...
function createTargetMap(vrm: VRM) {
  ...
  for (const [sourceName, humanBoneName] of Object.entries(sourceBoneToHumanBone)) {
    ...
    if (!node || !rawNode) {
      if (requiredHumanBoneNames.has(humanBoneName)) {
        missingRequiredTargets.push(`${sourceName} (${humanBoneName})`);
      }
      continue;
    }
  ...
  if (missingRequiredTargets.length > 0) {
    throw new Error(
      `Character VRM is missing required humanoid bones: ${missingRequiredTargets.join(", ")}`
    );
  }
  return targetMap;
}
